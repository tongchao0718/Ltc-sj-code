# power-fee-protocol-check SDD（V2.1）
...
- requirement_ref: 《01-需求说明书》V2.1（SA-1~SA-14，NFR-1~NFR-5）

### 1.1 技术与工程约束

- backend: Node.js + Express（AIEP-SERVER 工程）
- frontend: Vue 3 + Vue Router + Arco Design（子应用 `power-fee-protocol-check`）
- database: MongoDB（Mongoose，以仓库实际配置为准）
- middleware: Redis、Kafka（按场景启用，可选）
- architecture: 后端按 apps 模块划分 routes/controllers/services；前端按 views 页面拆分
- code_style: 接口返回结构统一；持久化字段命名与详细设计/库表文档对齐

## 2. 业务范围

- in_scope: 协议模板库、协议样本标注、核查规则、核查任务策略、协议识别核查执行、协议信息提取引擎、协议信息解析、识别结果校核、识别问题结果记录、问题分析及治理、核查任务监控、核查结果监控、协议识别能力优化、协同复核（对应 SA-1~SA-14）
- out_of_scope: 跨系统结算核心改造、财务记账改造（见需求说明书 5.5）
- business_goal: 建设「协议核查管理 → 异常分析治理 → 识别能力优化」全链路体系，形成识别、核查、治理与持续优化闭环
- success_metrics: 识别成功率、规则命中率、异常闭环率、任务成功率、关键字段错误率改善

## 2.1 需求边界与异常（摘要，对齐需求 5.5）

- 数据边界：营销系统为档案基准唯一主源；外部补录须标记来源与可信级别。  
- 最小异常覆盖：OCR 失败、表格错位、签章识别失败、规则引擎超时、任务中断、工单派发失败等；每类须定义重试、人工介入、回滚或降级，且可日志定位。  
- 新增需求须补齐需求说明书 5.4 矩阵中的输入/处理/输出/异常/验收五类信息。
...
- table_naming: 业务集合/表按语义命名，与《02-数据库设计文档》一致
- field_naming: 接口与持久化字段统一下划线命名（snake_case）；MongoDB 文档主键策略以详细设计为准
- common_columns: `created_at`、`updated_at`（必要时补充 `created_by`、`updated_by`）
- key_rule: 业务主键字段命名与实体一致，保证跨表关联可追溯
...
| AT-05 | 存量核查计划与资源配置就绪 | 分批执行协议识别核查 | 按计划完成率可核验且异常可闭环处理 |
| AT-06 | 增量对象提取规则已配置 | 执行营销增量/变动数据提取 | 形成增量核查对象集且提取过程可追踪 |
...
- 用户故事地图（第 4 节）须与《01-需求说明书》第 4 节活动、任务、对应需求编号及发布切片一致；差异须在需求说明书变更记录中说明。  
- 功能点 F-01~F-14 与 PRD 模块 M01~M14、验收 AT-01~AT-14 映射以《01-需求说明书》第 6 节追踪矩阵为准。  
- 页面与路由以 `04-页面-路由-接口-数据表映射表.md` 与产品设计信息架构为准。  
