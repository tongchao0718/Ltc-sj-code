...
> **Cursor 发现路径**：`.cursor/skills/prd-page-annotation/`（与本目录同步）
...
5. 角标为 **16px 圆形数字**（原型标注样式，不宜过大）
6. 生产构建默认关闭编辑（`VITE_PRD_ANNOTATION_EDIT=false`）
7. **隐藏/显示标注**：布局挂载 `PrdAnnotationToggle`，右下角一键切换；偏好写入 `localStorage`（刷新仍生效）
...
2. `annotations/{app}Annotations.js` 结构见 [reference.md](reference.md)
3. 环境变量：`VITE_PRD_ANNOTATION`、`VITE_PRD_ANNOTATION_EDIT`
...
- [ ] 浮窗：点击角标打开、标题栏拖拽、全屏/还原、仅 × 或再点角标关闭、450px 宽、Teleport 防裁剪
...
- 上游：`ui-generation`、`prd-design-generation`
- 预审：`design-review-pre-g2a`（D4）
- 走查：`ui-acceptance-review`
- 冻结：`prd-design-generation`（step4-freeze）
