...
| 一键复制 | 剪贴板（含 `app-code` 注释头 + `  {id}: {` 整段，UI 不展示） | 按 `target` 路径粘贴进对应子应用 `*.js` 后 Git 提交（无需 dev） |
| 同步到代码文件 | 直接写 `*Annotations.js` | 是（需 dev 服务） |
...
| 变量 | 默认（dev） | 说明 |
|------|-------------|------|
| `VITE_PRD_ANNOTATION` | 开启 | 显示角标与隐藏按钮 |
| `VITE_PRD_ANNOTATION_EDIT` | 开启 | 浮窗内编辑 |

用户一键隐藏：`PrdAnnotationToggle` → `localStorage`（`sample-app-prd-annotation-visible`）。

## 设计系统（CSS 常量）

### 角标 `.prd-badge`

16px 圆形数字角标（白边 + 轻阴影）；浮窗宽 450px；`z-index: 9999`。
...
| 打开 | 点击角标（再次点击角标或点 × 关闭） |
| 关闭 | 右上角 × 或再次点击角标 |
| 拖拽 | 标题栏拖动（全屏时不可拖，先点「还原」） |
| 尺寸 | 默认 450px；右上角「全屏」/「还原」切换 |
