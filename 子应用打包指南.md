...
1. **运行测试**：执行测试用例，确保代码质量和功能正常
2. **清理输出目录**：Vite 会自动清空 `dist/appX/` 目录
3. **编译源代码**：将 Vue 组件编译为 JavaScript
4. **打包资源**：将所有依赖打包为静态文件
5. **生成 HTML**：生成包含资源引用的 HTML 文件
6. **优化输出**：对输出文件进行压缩和优化
...
- **菜单链接处理**：在子应用的布局组件中，需要根据运行模式动态调整路由链接的前缀。例如：
  ...
  const base = computed(() => {
    // 检查是否在独立运行模式下
    const isStandalone = window.location.hash.startsWith('#/')
