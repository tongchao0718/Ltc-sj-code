...
| 功能ID | 功能名称 | 功能需求（必填） | 业务规则 | 优先级 | 前置条件 | 输出结果 | 验收引用 |
|------|------|------|------|------|------|------|------|
| F-01 |  | 对象+动作+结果（1~3条） | 关键约束（1~3条） | P0/P1/P2 |  |  | AT-01 |

### 5.1 功能点填写规则（强制）

- `功能需求` 必填，禁止空值或占位符。  
- 每条功能需求使用“对象 + 动作 + 结果”短句结构。  
- 每个功能点至少绑定 1 个 `验收引用`（`AT-xx`）。  
- `功能需求` 与第10章验收脚本必须可一一映射。  
