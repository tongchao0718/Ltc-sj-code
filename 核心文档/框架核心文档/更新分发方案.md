...
try {
  ...
  // 清理临时文件
  const tempFiles = [
    path.join(projectRoot, 'ltc-demo-update-v*.zip'),
    path.join(projectRoot, 'update-info.json'),
    path.join(projectRoot, 'update.js'),
    path.join(projectRoot, 'rollback.js')
  ]
  ...
  for (const tempFile of tempFiles) {
    if (fs.existsSync(tempFile)) {
      try {
        ...
        console.warn(`  ⚠ 清理临时文件失败: ${tempFile}`, error.message)
      }
    }
  }
...
1. **文件共享**
   ...
   - 用户解压后执行更新脚本
   - 离线包打包、命名、U 盘介质、完整性校验等细则统一遵循 `创建指南.md` 中“5.4 离线包打包与 U 盘分发规则”
...
1. **版本号更新要求**
   - 更新包创建时必须同步更新以下文件中的版本号：
     - `package.json`：更新 `version` 字段
     - `产品设计文档.md`：更新"关于我们"部分的版本号
     - `框架基线.md`：更新基线版本标识
     - `framework-baseline.json`：更新应用版本号
     - `AboutPage.vue`：更新页面显示的版本号
   - 确保所有版本号保持一致，避免版本号不一致导致的混淆

2. **版本更新流程**
   - 更新 `package.json` 中的版本号
   - 同步更新所有相关文档和代码文件中的版本号
   - 运行 `npm install` 确保依赖正确安装
   - 运行 `npm run build` 测试构建是否正常
   - 提交代码并创建版本标签
...
- 用户在解压前应验证文件完整性
- 离线交付的校验文件与交接说明要求统一遵循 `创建指南.md` 中“5.4 离线包打包与 U 盘分发规则”
...
5. **更新相关文档要求**
   - **必须更新的文档**：
     - 产品设计文档.md：更新版本号和功能描述
     - 安装指南.md：更新版本号和安装步骤
     - 框架基线.md：更新基线版本标识
     - 系统设计规范.md：更新设计规范和最佳实践
     - 文档对齐清单.md：更新文档对齐状态
     - 更新分发方案.md：更新分发流程和要求
     - 一键更新指南.md：更新更新步骤和注意事项
     - 主应用打包指南.md：更新打包配置和步骤
     - 子应用打包指南.md：更新子应用打包配置和步骤
     - 创建指南.md：更新创建流程和规范
     - 若涉及离线/U盘交付流程，必须同步更新“创建指南.md（5.4 离线包打包与 U 盘分发规则）”与“更新分发方案.md（本章）”
   - **更新时机**：每次版本更新时必须同步更新相关文档
   - **更新内容**：
     - 版本号更新
     - 新功能说明
     - 变更的流程和步骤
     - 新增的依赖和环境要求
     - 修复的问题和改进的功能
   - **验证要求**：更新后必须验证文档的准确性和完整性，确保文档与代码实现一致
